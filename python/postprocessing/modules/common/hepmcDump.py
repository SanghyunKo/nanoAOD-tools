...
class hepmcDump(Module, object):
    def __init__(self, pdfStart, pdfEnd ,*args, **kwargs):
        ...
        if "/hepmcDumpCppWorker_cc.so" not in ROOT.gSystem.GetLibraries():
            ...
            if base:
                ROOT.gROOT.ProcessLine(".L %s/src/hepmcDumpCppWorker.cc+O" %
                                       base)
            else:
                base = "%s/src/PhysicsTools/NanoAODTools" % os.getenv(
                    "CMSSW_BASE")
                ROOT.gSystem.Load("libPhysicsToolsNanoAODTools.so")
                ROOT.gROOT.ProcessLine(".L %s/interface/hepmcDumpCppWorker.h" %
                                       base)
        pass
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        super(hepmcDump, self).beginFile(inputFile, outputFile, inputTree,
                                         wrappedOutputTree)
    ...
    def initReaders(self, tree):
        ...
        self.GenPart_genPartIdxMother = tree.arrayReader(
            "GenPart_genPartIdxMother")
        self.worker.setGenParticles(self.nGenPart, self.GenPart_pt,
                                    self.GenPart_eta, self.GenPart_phi,
                                    self.GenPart_mass, self.GenPart_pdgId,
                                    self.GenPart_status,
                                    self.GenPart_genPartIdxMother)

        self._ttreereaderversion = tree._ttreereaderversion
        pass

    def analyze(self, event):
        """process event, return True (go to next module) or False
        (fail, go to next event)"""
